...
%\affiliation{ADP Innovation Labs}
...
%Every big data manager should ask themselves these important questions. However, we won't explore data management further as this is a technique oriented book. 
...
Similar to Shark, Spark SQL employs an in-memory columnar store. Different form Shark, Spark SQL does not use any query optimizations of Hive. Hive's query optimizer has a lot of complexity to address the limitations of MapReduce. But many of those do not apply in Spark. So Spark SQL designs a new query optimizer framework Catalyst.
...
The basic unit of sharding is called a Region\index{HBase!Region} in HBase. A region is a contiguous and sorted range of rows of a table stored together on disk. Initially, there is only one region for a table. However, when regions become two large, a region is split into two at the middle key (recall that rows are lexicographically sorted by row keys). Regions are served by RegionServer\index{HBase!RegionServer}. Each RegionServer is responsible a set of regions but one region can be served only by one RegionServer.
...
To store a large number of keys, LevelDB is preferred. LevelDB\index{LevelDB} is an open source on-disk key-value store \cite{LevelDB}\index{LevelDB}. It shares the same general design as the BigTable tablet stack and is written by Google fellows by Jeffrey Dean and Sanjay Ghemawat. LevelDB supports batching writes, forward and backward iteration, and compression of the data via Google's Snappy compression library. We will discuss LevelDB in details later in its own section.
...
\chapter{Cluster Management and DevOp}
